...
float mapLMSemBuffer(){
  ...
  float tempC;
  double value;
  value = analogRead(A12); // Leitura do sensor LM35
...
void loop() {
   ...
   bufferCircular[posicao] = analogRead(A12); // Realiza leitura do sensor LM35 (A12) e armazena no buffer
   posicao = (posicao +1) % BUFFERSIZE; // Altera o index do buffer. O calculo resulta 
   /////////////////////////////////////// sempre em um valor de 1 até BUFFERSIZE
   
   mediaBuffer(); // Calcula a media do buffer
   temperatura = mapLM(); // Realiza o mapeamento da leitura de temperatura
   ...
   //lcd.print(temperaturaSemBuffer); 
   lcd.print("C");

   //########################### Controle do LED
   if(temperatura > 30){
      digitalWrite(led, HIGH);
      //Comentando a linha acima e descomentando as próximas,
      //o LED passa a piscar quando atinge a temperatura configurada (30º)
